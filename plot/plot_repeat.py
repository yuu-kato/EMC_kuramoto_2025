...
def calc_stat(param, subj_list, subj, K_est, gamma_est, K_variance, gamma_variance):
    ...
    K_index = np.abs(K_est - K_true) / np.sqrt(K_variance)
    gamma_index = np.abs(gamma_est - gamma_true) / np.sqrt(gamma_variance)

    # prepare empty arrays
    K_MAP_ave, gamma_MAP_ave, K_MAP_std, gamma_MAP_std, K_MAP_dif_ave, gamma_MAP_dif_ave, K_MAP_dif_std, gamma_MAP_dif_std, K_var_ave, gamma_var_ave, K_var_std, gamma_var_std, K_index_mean, K_index_std, gamma_index_mean, gamma_index_std = [np.array([]) for _ in range(16)]
    ...
    for i in subj_list:
        ...
        K_index_mean = np.append(K_index_mean, np.mean(K_index[np.where(subj == i)]))
        K_index_std = np.append(K_index_std, np.std(K_index[np.where(subj == i)]))
        gamma_index_mean = np.append(gamma_index_mean, np.mean(gamma_index[np.where(subj == i)]))
        gamma_index_std = np.append(gamma_index_std, np.std(gamma_index[np.where(subj == i)]))

    return K_MAP_ave, gamma_MAP_ave, K_MAP_std, gamma_MAP_std, K_MAP_dif_ave, gamma_MAP_dif_ave, K_MAP_dif_std, gamma_MAP_dif_std, K_var_ave, gamma_var_ave, K_var_std, gamma_var_std, K_index_mean, K_index_std, gamma_index_mean, gamma_index_std
...
def change_set(mode):
    if mode=='OA':
        ...
        ylim_index = [0, 4.8]
        xlabel = r'$N$'
    return XLIM, ylim_index, xlabel

# Plot figure
def plot_rep(param, subj_list, K_MAP_ave, gamma_MAP_ave, K_MAP_std, gamma_MAP_std, K_MAP_dif_ave, gamma_MAP_dif_ave, K_MAP_dif_std, gamma_MAP_dif_std, K_var_ave, gamma_var_ave, K_var_std, gamma_var_std, K_index_mean, K_index_std, gamma_index_mean, gamma_index_std, XLIM, ylim_index, xlabel, savefilename):
    K_true = param['K_true'][0]
    gamma_true = param['gamma_true'][0]
    ...
    ax5.set_ylabel('Variance of posterior', fontsize = labelsize)
    ...
    ax7.axhspan(0, 1, color="gray", alpha = 0.5)
    ax7.errorbar(subj_list, K_index_mean, yerr=K_index_std, color = myblue, marker=".", markersize=20, label = r'$\frac{|K_{\rm true}-\hat{K}|}{{\rm Std}\: [\hat{p}(K|D,b_{\hat{l}})]}$', capthick=1, capsize=5, lw=3)
    ax7.set_xscale('log')
    ax7.set_xlim(XLIM)
    ax7.set_ylim(ylim_index)
    ax7.set_ylabel("Difference / SD", fontsize = labelsize)
    ax7.legend(fontsize = legendsize2)
    ax7.set_xlabel(xlabel, fontsize = labelsize)
    ax7.text(-0.02, 1.12, '(g)', horizontalalignment='right', verticalalignment='top', transform=ax7.transAxes, fontsize=panellabelsize)

    ax8 = fig.add_subplot(428)
    ax8.axhspan(0, 1, color="gray", alpha = 0.5)
    ax8.errorbar(subj_list, gamma_index_mean, yerr=gamma_index_std, color = mygreen, marker=".", markersize=20, label = r'$\frac{|\gamma_{\rm true}-\hat{\gamma}|}{{\rm Std}\: [\hat{p}(\gamma|D,b_{\hat{l}})]}$', capthick=1, capsize=5, lw=3)
    ax8.set_xscale('log')
    ax8.set_xlim(XLIM)
    ax8.set_ylim(ylim_index)
    ax8.legend(fontsize = legendsize2)
    ax8.set_xlabel(xlabel, fontsize = labelsize)
    ax8.text(-0.02, 1.12, '(h)', horizontalalignment='right', verticalalignment='top', transform=ax8.transAxes, fontsize=panellabelsize)

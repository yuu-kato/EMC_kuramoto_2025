...
// Repeated EMC simulations for a fixed dataset
// The case when oscillator number = 100000
...
void EMC(int L, const vector<double> &b, int N, const vector<double> &t_data, const vector<double> &R_data, vector<vector<double>> s, int simulation_size, int burn_in, double target_accept, double range_accept, int seed, int exchange_freq, double alpha_gamma, double beta_gamma, double alpha_delta_gamma, double beta_delta_gamma, ofstream &fout_ext){
    ...
    ofstream fout_3("./output_files_kuramoto_n100000_error/exchange_rate.csv");
    ofstream fout_4("./output_files_kuramoto_n100000_error/acceptance_rate.csv");
    ofstream fout_5("./output_files_kuramoto_n100000_error/step_size.csv");
    fout_3 << "mc_step" << ",";
    fout_4 << "mc_step" << ",";
    fout_5 << "mc_step" << ",";
    for (int l = 0; l < L; l++) {
    ...
    ofstream fout_6("./output_files_kuramoto_n100000_error/MAP.csv");
    fout_6 << "l" << ",";
    for (int i = 0; i < L; i++) {
        fout_6 << i << ",";
    }
    ...
    ofstream fout_7("./output_files_kuramoto_n100000_error/free_energy.txt");
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    // recording error
    int total_steps = simulation_size /10 + 1;
    vector<vector<double>> error_mat_all(L ,vector<double>(total_steps)); 
    ...
    for (int i = 0; i < L; i++) {
        ...
        error_mat_all.at(i).at(0) = error.at(i); // initial error
    }
    theta_MAP = theta;
    ...
    for (int MC_step = 1; MC_step <= simulation_size ; MC_step++){
        ...
        if (MC_step % 10 == 0){
            ...
            for (int l = 0; l < L; l++){
                ...
                error_mat_all.at(l).at(MC_step/10) = error.at(l);
            }
            fout_1 << endl;
        }
    ...
    for (int l = 0; l < L; l++){
        for (int k = (burn_in/10)+1; k < total_steps; k++){
        error_mat.at(l).at(k - (burn_in/10) - 1) = error_mat_all.at(l).at(k);
        }
    }
    ...
    fout_ext << l_index << ",";
    for (int n = 0; n < total_steps-1; n++){
        fout_ext << error_mat_all.at(l_index).at(n) << ",";
    }
    fout_ext << error_mat_all.at(l_index).at(total_steps-1) << endl;
}
...
int main() {
    ...
    // output files
    mkdir("./output_files_kuramoto_n100000_error", 0755); // 0755 means 'allowing the owner to write, read, execute, while allowing others only to read and execute'
    ofstream fout_1("./output_files_kuramoto_n100000_error/data.txt");
    ofstream fout_2("./output_files_kuramoto_n100000_error/true_parameters.csv");
    ofstream fout_3("./output_files_kuramoto_n100000_error/error.csv");
    ...
    int M = 100000; // number of oscillators
    int N = 101; // number of data points
    double t_max = 50.0; // max time
    double dt = 0.001; 
    int seed = 4032; // seed of random number
    ...
    int rep_num = 100; // number of EMC simulations to visualize error dynamics

    // record parameters 
    fout_2 << "gamma_true" << "," << "K_true" <<  "," << "mu" << "," << "t_max" << "," << "M" << "," << "N" << "," << "dt" << "," << "L_rep" << "," << "b_max" << "," << "b_ratio" << "," << "alpha_gamma" << "," << "beta_gamma" << "," << "alpha_delta_gamma" << "," << "beta_delta_gamma" << "," << "C_gamma" << "," << "C_delta_gamma" << "," << "d_gamma" << "," << "d_delta_gamma" << "," << "exchange_freq" << "," << "simulation_size"  << "," << "burn_in" << "," << "targer_accept" << "," << "range_accept" << "," << "rep_num" << "," << "true_error" << endl;

    fout_2 << gamma_true << "," << K_true << "," << mu << "," << t_max << "," << M << "," << N << "," << dt << "," << L_rep << "," << b_max << "," << b_ratio << "," << alpha_gamma << "," << beta_gamma << "," << alpha_delta_gamma << "," << beta_delta_gamma << "," << C_gamma << "," << C_delta_gamma << "," << d_gamma << "," << d_delta_gamma << "," << exchange_freq << "," << simulation_size << "," << burn_in << "," << target_accept << "," << range_accept << "," << rep_num << ",";

    fout_3 << "mc_step" << ",";
    for (int n = 0; n < simulation_size/10; n++){
        fout_3 << 10*n << ",";
    }
    fout_3 << simulation_size << endl;
    ...
    // calculate and record true error
    double err_true;
    vector<double> theta_true(2);
    theta_true.at(0) = gamma_true;
    theta_true.at(1) = gamma_true - (K_true/2);
    err_true= error_func(N, t_data, R_data, theta_true);
    fout_2 << err_true << endl;
    ...
    for (int p = 0; p < rep_num; p++){
        EMC(L_rep, b, N, t_data, R_data, s, simulation_size, burn_in, target_accept, range_accept, seed + p, exchange_freq, alpha_gamma, beta_gamma, alpha_delta_gamma, beta_delta_gamma, fout_3);
    }
